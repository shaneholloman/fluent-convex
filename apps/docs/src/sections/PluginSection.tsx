...
export function PluginSection() {
  ...
  const taggedResult = useQuery(api.plugin.taggedQuery as any, { echo: echoInput });
  ...
  return (
    <section id="custom-plugins" className="flex flex-col gap-6">
      ...
      <Prose>
        ...
        <p>
          The example below defines a <code className="bg-slate-200 dark:bg-slate-800 px-1 py-0.5 rounded text-sm">TaggedBuilder</code>{" "}
          plugin that adds a{" "}
          <code className="bg-slate-200 dark:bg-slate-800 px-1 py-0.5 rounded text-sm">.withTag(name)</code>{" "}
          method. Calling it wraps the function in an onion middleware that logs start/end events.
        </p>
      </Prose>
      <CodeBlock source={pluginSource} region="TaggedBuilder" title="convex/plugin.ts - defining a plugin" file="convex/plugin.ts" />
      ...
      <Prose>
        <p>
          ...
          <code className="bg-slate-200 dark:bg-slate-800 px-1 py-0.5 rounded text-sm">.extend(TaggedBuilder)</code> and
          then call your custom method. The rest of the chain works as normal.
        </p>
      </Prose>
      <CodeBlock source={pluginSource} region="taggedQuery" title="convex/plugin.ts - using the plugin" file="convex/plugin.ts" />
      <DemoCard title="Live demo">
        ...
        <p className="text-xs text-slate-400 dark:text-slate-500">
          Check the Convex dashboard logs to see the logging output from the plugin.
        </p>
      </DemoCard>
    </section>
  );
